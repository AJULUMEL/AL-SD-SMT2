...
public class PencarianBuku06 {
    ...
    public int FindBinarySearchJudul(String cari, int left, int right) {
        while (right >= left) {
            ...
            if (listBk[left].judulBuku.toLowerCase().compareTo(cari.toLowerCase()) <= 0
                    && cari.toLowerCase().compareTo(listBk[mid].judulBuku.toLowerCase()) <= 0) {
                right = mid - 1;
            } else {
                left = mid + 1;
            }
    ...
    public void TampilDataBuku(String judul) {
        ...
        if (totalDitemukan > 1) {
            System.out.println(
                    "Peringatan: Ditemukan " + totalDitemukan + " judul buku yang mengandung kata \"" + judul + "\".");
